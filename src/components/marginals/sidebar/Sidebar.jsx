...
} from 'lucide-react'; // ShadCN uses Lucide icons
...
const Sidebar = () => {
  ...
  const navItems = {
    ...
    athlete: [
      ...
      {
        name: 'Tracking',
        path: '/dashboard/athlete/performance_evaluation',
    ...
    organization: [
      { name: 'Dashboard', path: '/dashboard/organization', icon: BarChart },
      {
        ...
        path: '/dashboard/organization/athlete_management',
        icon: Settings,
      },
      { name: 'Events', path: '/events', icon: Calendar },
    ],
        ...
        <nav className='flex flex-col space-y-4 pr-[20px]'>
          {activeNav.map(({ name, path, icon: Icon }) => (
            <Link
              key={name}
              href={path}
              className='flex items-center space-x-2 py-2 pl-[5px] hover:bg-[#9f87b6a1] rounded'
            >
              <Icon size={20} />
          ...
          {user && (
            <Button
              ...
              className='flex items-center justify-center gap-3 p-5 mt-auto hover:bg-red-600 rounded-3xl'
            >
